...
public class LogContext : DbContext
{
    ...
    {
        optionsBuilder.UseSqlServer("Server=PC03LAB2532\\SENAI;Database=WebScrapingDb2;User Id=sa;Password=senai.123;"); // Substitua "YourConnectionString" pela sua string de conexão
    }
}
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    foreach (Produto produto in novosProdutos)
                    {
                        if (!produtosVerificados.Exists(p => p.Id == produto.Id))
                        {
                            ...
                            {
                                RegistrarLog("001897", "wallacemaximus", DateTime.Now, "ConsultaAPI - Verificar Produto", "Sucesso", produto.Id);
